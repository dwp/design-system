...
const { removeWhitespace } = require('../utils/regexes')
...
exports.getHtmlCode = (path) => {
  ...
  return beautify(html.replace(removeWhitespace,''), {
...
// returns CSS from path provided 
exports.getCSSCode = (path) => {
  let fileContents;
  try {
    const file = this.getFileContents(path);
    const parsedFile = matter(file);
    fileContents = beautify(parsedFile.content);
  } catch(err) {
    if(err){
      return null;
    }
